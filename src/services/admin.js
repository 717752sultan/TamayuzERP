...
const roleToDb = (role = {}) => ({
  ...
  role_name: normalizeRoleName(role.role_name || role.name || ""),
...
export const adminService = {
  ...
  async saveRole(role) {
    try {
      ...
      if (!payload.role_name || payload.role_name === "غير محدد") throw new Error("يجب إدخال اسم الدور");
  ...
  async loadEmployeesForUserDropdown() {
    try {
      ...
      return (rows || [])
        .map(employeeFromDb)
        .filter((row) => row.id && row.name && (!row.status || row.status === "نشط"));
    } catch (error) {
