    ...
    <div className="relative grid min-h-screen place-items-center overflow-hidden bg-[#111319] p-5">
      ...
      <div className="relative grid w-full max-w-5xl overflow-hidden rounded-[28px] bg-white shadow-2xl md:grid-cols-2">
        ...
        <form
          onSubmit={(e) => {
            e.preventDefault();
            const users = settings.users || defaultSettings.users;
            const user = users.find(
              (item) => item.username === u && item.password === pw,
            );
            user
              ? onLogin(user)
        ...
        >
          ...
          <button className="btn-primary mt-7 h-12 w-full">
            دخول إلى النظام <ArrowUpLeft size={18} />
          </button>
          <p className="mt-5 text-center text-xs text-slate-400">
            الإدارة: admin / 1234 — الموظف: employee / 1234
          </p>
        </form>
      </div>
    </div>
