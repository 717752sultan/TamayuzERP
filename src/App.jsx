...
function DailyOperationsPageEnhanced({ employees = [], currentUser, currentCompany, can }) {
  ...
  const totalOperations = kpiRows.reduce((total, row) => total + Number(row.operation_count || 0), 0);
...
function DailyOperationsPage({ employees, currentUser, can }) {
  ...
  const approvedProductivityRows=filtered.filter((row)=>isApprovedDailyOperation(row));
  // All productivity reports use approved daily operations only.
  const totalOps = approvedProductivityRows.reduce((s, x) => s + Number(x.operation_count || 0), 0), totalErrors = filtered.reduce((s, x) => s + Number(x.error_count || 0), 0);
