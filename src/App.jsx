...
function DailyOperationsPageEnhanced({ employees = [], currentUser, currentCompany, can }) {
  ...
  const allStatusOperations = sum("operation_count");
  ...
  const approvedProductivityRows = Array.isArray(filtered)
    ? filtered.filter((row) => isApprovedDailyOperation(row))
    : [];
  // All productivity reports use approved daily operations only.
  const totalOperations = approvedProductivityRows.reduce((total, row) => total + Number(row.operation_count || 0), 0);
  ...
  const byBranch = Object.entries(groupCount(approvedProductivityRows, "branch")).map(([name, value]) => ({ name, value }));
...
function DailyOperationsPage({ employees, currentUser, can }) {
  ...
  const approvedRowsForProductivity = Array.isArray(filtered)
    ? filtered.filter((row) => isApprovedDailyOperation(row))
    : [];
  // All productivity reports use approved daily operations only.
  const totalOps = approvedRowsForProductivity.reduce((s, x) => s + Number(x.operation_count || 0), 0), totalErrors = filtered.reduce((s, x) => s + Number(x.error_count || 0), 0);
  const byBranch = Object.entries(groupCount(approvedRowsForProductivity, "branch")).map(([name, value]) => ({ name, value }));
