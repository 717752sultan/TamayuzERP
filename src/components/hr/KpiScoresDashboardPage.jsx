  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/50 p-4" dir="rtl">
      <div ref={ref} className="mx-auto max-w-6xl space-y-5 rounded-[2rem] bg-slate-100 p-4">
        ...
        <OfficialReportLayout title="تقرير تقييم أداء الموظف" company={company} period={month} generatedBy={currentUser?.username} filters={[`الموظف: ${report.employee_name}`, `الرقم الوظيفي: ${report.employee_id}`, `الوظيفة: ${report.job_name}`, `الفرع: ${report.branch}`, `الإدارة: ${report.department || "—"}`]} summary={summary}>
...
export default function KpiScoresDashboardPage({ employees = [], currentCompany, currentUser }) {
  ...
  return (
    <div className="space-y-5" dir="rtl">
      ...
      {tab === "ترتيب الموظفين" && <div className="panel overflow-x-auto p-4"><table className="w-full text-sm"><thead><tr>{["الترتيب", "الموظف", "الرقم الوظيفي", "الوظيفة", "الفرع", "إجمالي العمليات", "قبض", "صرف", "بيع", "شراء", "درجة KPI", "التقدير", "نقاط القوة", "ملاحظات", "إجراء"].map((h) => <th key={h}>{h}</th>)}</tr></thead><tbody>{ranking.length ? ranking.map((row) => <tr key={row.employee_id}><td>{row.rank}</td><td><button onClick={() => openReport(row)} className="font-extrabold text-brand-700">{row.employee_name}</button></td><td>{row.employee_id}</td><td>{row.job_name}</td><td>{row.branch}</td><td>{row.operations?.total_operations || 0}</td><td>{row.operations?.receipt_operations || 0}</td><td>{row.operations?.payment_operations || 0}</td><td>{row.operations?.sale_operations || 0}</td><td>{row.operations?.purchase_operations || 0}</td><td>{scoreText(row.final_score)}</td><td><span className={`rounded-full px-3 py-1 text-xs font-bold ${toneClass[row.performance_tone]}`}>{row.performance_label}</span></td><td>{row.strengths}</td><td>{row.notes}</td><td><button onClick={() => openReport(row)} className="btn-secondary !h-8"><Eye size={15} /> عرض التقرير</button></td></tr>) : <tr><td colSpan={15} className="py-8 text-center text-slate-400">لا توجد بيانات تقييم ضمن الفترة المحددة.</td></tr>}</tbody></table></div>}
      {tab === "أفضل 10 موظفين" && <div className="grid gap-5 xl:grid-cols-2"><BarList title="أفضل 10 موظفين حسب KPI" rows={topByScore} valueKey="final_score" /><BarList title="أفضل 10 موظفين حسب عدد العمليات" rows={topByOperations} valueKey="total_operations" max={Math.max(1, topByOperations[0]?.operations?.total_operations || 1)} /></div>}
