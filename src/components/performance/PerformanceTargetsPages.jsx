...
function TargetsPage({ kind, employees = [], currentCompany }) {
  ...
  const toggleSelected=id=>{if(!id)return;setSelectedIds(prev=>prev.includes(id)?prev.filter(x=>x!==id):[...prev,id])};
  ...
  const saveBulkEdit=async()=>{
    if(!selectedIds.length){setMessage("\u064a\u0631\u062c\u0649 \u062a\u062d\u062f\u064a\u062f \u0633\u062c\u0644 \u0648\u0627\u062d\u062f \u0639\u0644\u0649 \u0627\u0644\u0623\u0642\u0644");return}
    const patch={};
    Object.keys(bulkEnabled).filter(key=>bulkEnabled[key]).forEach(key=>{const value=bulkDraft?.[key];if(key==="is_active")patch[key]=value===true||value==="true"||value==="\u0645\u0641\u0639\u0644";else patch[key]=value});
    if(!Object.keys(patch).length){setMessage("\u064a\u0631\u062c\u0649 \u0627\u062e\u062a\u064a\u0627\u0631 \u062d\u0642\u0644 \u0648\u0627\u062d\u062f \u0639\u0644\u0649 \u0627\u0644\u0623\u0642\u0644 \u0644\u0644\u062a\u0639\u062f\u064a\u0644");return}
    try{setSavingEdit(true);const result=await api.updateEmployeeTargetsBulk(companyId,selectedIds,patch);setMessage(result.message);if(!result.success||!result.affectedCount)return;setSelectedIds([]);setBulkDraft(null);setBulkEnabled({});load()}catch(error){console.error("Employee targets bulk update error:",error);setMessage("\u062a\u0639\u0630\u0631 \u062a\u0637\u0628\u064a\u0642 \u0627\u0644\u062a\u0639\u062f\u064a\u0644 \u0627\u0644\u062c\u0645\u0627\u0639\u064a")}finally{setSavingEdit(false)}
  };
