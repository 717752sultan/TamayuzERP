...
export default function EmployeeAttendancePage({ company, employeeId, user }) {
  ...
  const load = async () => {
    ...
    setProfile(employee);
    setAppSettings(await employeeAppService.loadEmployeeAppSettings(company.company_id).catch(() => ({ geofence_required: true, allow_attendance_without_location: false })));
  ...
  return <div className="space-y-4"><section className="rounded-[2rem] bg-white p-5 shadow-sm"><div className="mb-4 flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-violet-100 text-violet-700"><MapPin /></div><div><h2 className="text-xl font-black">تسجيل الحضور بالموقع</h2><p className="text-sm text-slate-500">{today} • {currentStatus}</p></div></div><div className="rounded-2xl bg-amber-50 p-3 text-xs font-bold text-amber-800">تسجيل الحضور بالموقع يعتمد على إذن الموقع ودقة GPS، ويتم حفظ بيانات الموقع لأغراض التحقق الإداري.</div>{message && <div className="mt-3 rounded-2xl bg-slate-50 p-3 text-sm font-bold text-slate-700">{message}</div>}<div className="mt-5 grid gap-3 md:grid-cols-2"><button disabled={saving || status.hasCheckIn} onClick={() => submit("check_in")} className="rounded-3xl bg-violet-700 p-5 text-lg font-black text-white disabled:opacity-50">تسجيل دخول</button><button disabled={saving || !status.hasCheckIn || status.hasCheckOut} onClick={() => submit("check_out")} className="rounded-3xl bg-slate-800 p-5 text-lg font-black text-white disabled:opacity-50">تسجيل خروج</button></div></section><section className="rounded-[2rem] bg-white p-5 shadow-sm"><h3 className="mb-3 flex items-center gap-2 font-black"><ShieldCheck size={18} /> سجل الحضور</h3>{history.length ? <div className="space-y-2">{history.map((event) => <div key={event.event_id} className="flex flex-wrap justify-between gap-2 rounded-2xl bg-slate-50 p-3 text-sm"><b>{event.event_type === "check_in" ? "دخول" : "خروج"}</b><span>{event.attendance_date}</span><span>{timeLabel(event.event_time)}</span><span>{event.geofence_status === "inside" ? "داخل النطاق" : event.geofence_status === "outside" ? "خارج النطاق" : "غير متاح"}</span></div>)}</div> : <p className="text-sm text-slate-400">لا توجد أحداث حضور</p>}</section></div>;
}
